...
| M4 | One-trial-only (`billingPolicy`) | `billing_state` amb `trialConsumedAt` posat → crida checkout | `selectCheckoutPrice` retorna price estàndard, mai `<base>_trial`; verificar també els **8 lookup_keys exactes** a Stripe TEST (individual/club × mensual/anual × trial/estàndard) | Reset `billing_state` |
