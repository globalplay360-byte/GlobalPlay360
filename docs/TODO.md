...
- [ ] **No editar plans des del panell admin**: decisió de producte per a l'MVP. Si la clienta vol canviar el pla d'un usuari, s'ha de fer a Stripe perquè és la font de veritat. No reutilitzar `users.plan` com a override manual. Si en el futur es necessita concedir accés intern manual, dissenyar un sistema separat i auditat (`adminGrantedPlan`, expiració, motiu) en lloc de tocar la subscripció mirrored.
- [ ] **Optimització del bundle inicial del frontend**: el build actual genera un chunk principal JS d'aproximadament `9.95 MB` (`~2.69 MB gzip`), acceptable temporalment per a la landing pre-launch però massa pesat per al llançament públic.
  - **Deixar per a la següent sessió** com a pendent de performance, no blocker d'entrega.
  - **Símptoma actual detectat:** `build_output/assets/bootstrap-Bbo2718D.js` surt especialment gran al build de producció.
  - **Primer pas recomanat en reprendre-ho:** aplicar lazy loading de rutes a `src/App.tsx` amb `React.lazy` + `Suspense` per separar pàgines públiques, dashboard i admin.
  - **Segon pas:** repetir `npm run build` i comparar el pes final dels chunks.
  - **Tercer pas, només si encara continua alt:** revisar dependències grans i valorar `manualChunks` a Vite per dividir millor el codi inicial.
